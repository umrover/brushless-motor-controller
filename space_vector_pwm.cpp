...
//	uint32_t ARR_C = PhaseC_iface.TIM_obj->ARR;
	...
	this->sector = calculate_sector(); //function assumes voltageAlpha and voltageBeta have been set
	calculate_XYZ();
	calculate_relative_time_on(this->sector, PWM_Period_A);
...
void SpaceVectorPWM::writePWM(){
	...
	uint32_t ARR_phase_A = 65535;
	uint32_t ARR_phase_B = 65535;
